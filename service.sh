...
while true; do
    ...
    # Read and flatten config JSON to handle multi-line formatting safely
    ...
    CURRENT_DAY=$(date '+%a')
    IS_DAY_ACTIVE=$(echo "$CFG_FLAT" | grep -o '"active_days"[[:space:]]*:[[:space:]]*\[[^]]*\]' | grep -o "$CURRENT_DAY")
        ...
        if [ "$INTERVAL_SEC" -gt 0 ]; then
            ...
            if [ "$ELAPSED" -ge "$INTERVAL_SEC" ]; then
                ...
                if [ "$IS_LOCKED" -eq 0 ]; then
                    # Execute action script in organize mode
                    if [ -f "$SETTINGS_DIR/action.sh" ]; then
                        sh "$SETTINGS_DIR/action.sh" organize >/dev/null 2>&1
                    elif [ -f "$MODDIR/action.sh" ]; then
                        sh "$MODDIR/action.sh" organize >/dev/null 2>&1
                    fi
                    NOW_END=$(date +%s)
    ...
    # Dynamic sleep quantum: sleep 2s if interval <= 5s, else 5s
    SLEEP_TIME=5
